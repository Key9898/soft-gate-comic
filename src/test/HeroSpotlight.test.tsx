...
describe('HeroSpotlight', () => {
  ...
  it('renders the first trending slide title', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Alpha Trending' })).toBeInTheDocument()
  })
  ...
  it('jumps to a slide when a dot is clicked', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Beta Trending' })).toBeInTheDocument()
  })
  ...
  it('autoplays after the interval and pauses on hover', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Gamma Trending' })).toBeInTheDocument()
    vi.useRealTimers()
  })
  ...
  it('keeps HeroBook3D under a plain width wrapper (no Framer parent)', () => {
    ...
    expect(scene?.className).toMatch(/\bhero-book-enter\b/)
    ...
    expect(parent?.querySelector(':scope > .hero-book-scene')).toBe(scene)
    ...
    const heading = screen.getByRole('heading', { name: 'Alpha Trending' })
    expect(heading.parentElement?.parentElement?.className).toMatch(/\bz-10\b/)
    expect(heading.parentElement?.parentElement?.className).toMatch(/\brelative\b/)
  })
  ...
  it('clamps hero title and deck without slicing catalog copy', () => {
    ...
    expect(heading.className).toMatch(/\bline-clamp-2\b/)
    expect(heading.className).toMatch(/\blg:line-clamp-1\b/)
