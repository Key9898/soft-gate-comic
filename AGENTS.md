...
- Shared package: `packages/shared` (`@softgate/shared`) for catalog types/mock data. Contracts: `packages/contracts` (`@softgate/contracts`). API: `apps/api` (`@softgate/api`) — `GET /health`, `GET /api/catalog`, `GET /api/settings`, `GET /api/about`, `GET /api/press`, `GET /api/legal/privacy`, `GET /api/legal/terms`, `GET /api/faq`, `GET /api/cookies`, reader `/api/auth/*`, `/api/wallet/*`, `/api/library/*`, `/api/notifications/*`, `/api/prefs/*`, `/api/comments`, and `/api/internal/notifications` when portal mock is off (Impl 174–175, 187–192, 197, 203–205, 211–213). Named Prisma/R2/Brevo/VAPID env slots (Impl 176); Prisma persist when `DATABASE_URL` is set (Impl 185); R2 put helper under `portal/` when core slots are set (Impl 186); Brevo HTML forgot/reset when key+from are set (Impl 187). Leader-dev JWT/R2/Brevo map onto those slots in gitignored local env (Impl 193). Local `DATABASE_URL` + `prisma migrate deploy` once (Impl 194). Prisma persist maps Admin catalog tables on `GET /api/catalog` (Impl 195); stub persist stays seed. Empty published catalog keeps live Home/Categories/Search chrome (Impl 196). Catalog load-fail is not unpublished (Impl 198). Prisma persist maps Admin `PlatformSettings` on `GET /api/settings` (Impl 200); stub stays seed. Prisma persist maps Admin `CoinPackage` onto catalog `coinPackages` (Impl 201); missing table omits the field. Prisma persist maps Admin About CMS on `GET /api/about` (Impl 205); empty lists stay `[]`; meta missing/`P2021` fail-opens; portal `/about` History consumes `GET /api/about` (Impl 206); portal `/about` Team consumes `members` + `meta` on the same GET (Impl 207). Prisma persist maps Admin Press CMS on `GET /api/press` (Impl 211); missing table fail-opens to stub; portal `/press` consumes it when mock is off and keeps `t('press.*')` on fail. Prisma persist maps Admin Privacy/Terms CMS on `GET /api/legal/privacy` and `GET /api/legal/terms` (Impl 212); missing table fail-opens to stub; portal `/privacy` `/terms` consume it when mock is off and keep `t('static.*')` on fail. Prisma persist maps Admin FAQ/Cookies CMS on `GET /api/faq` and `GET /api/cookies` (Impl 213); missing table fail-opens to stub; portal `/faq` `/cookies` consume it when mock is off and keep today’s i18n on fail. Help hub stays catalog. Live join (Impl 202): Admin Express often `:3000`; SoftGate gitignored `PORT` + portal `VITE_API_BASE_URL`. Shared comments thread is mock unless `VITE_USE_MOCK_API=false` (Impl 197). Portal catalog/auth/wallet/library/notifications/prefs is mock unless `VITE_USE_MOCK_API=false` (`isMockApi` unset stays mock). Committed `apps/portal/.env.example` is `false`; Vite does not load the example. Local `pnpm dev` HTTP needs gitignored `.env.development.local` plus `pnpm dev:api`. Profile writers are cookie POSTs in HTTP mode (Impl 188). Library Subscribe/History/Likes are cookie persist in HTTP mode (Impl 190). Notifications inbox is cookie persist in HTTP mode (Impl 191). Admin service-token broadcast + Web Push + comment_reply email/push (Impl 203) and `new_episode` fan-out (Impl 204, `wiki/conventions/portal-notifications-deliver.md`). Notif toggles and reader display prefs are cookie persist in HTTP mode (Impl 192). Comments are a shared thread on `/api/comments` in HTTP mode (Impl 197). Git: `main` is product/UI and Vercel; `development` is the leader-dev integration track (local gitignored env only).
