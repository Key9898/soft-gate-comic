...
- Shared package: `packages/shared` (`@softgate/shared`) for catalog types/mock data. Contracts: `packages/contracts` (`@softgate/contracts`). API: `apps/api` (`@softgate/api`) — `GET /health`, `GET /api/catalog`, `GET /api/settings`, reader `/api/auth/*` and `/api/wallet/*` when portal mock is off (Impl 174–175). Named Prisma/R2/Brevo env slots + schema (Impl 176); persist still stub. Default portal catalog/auth/wallet is still localStorage.
