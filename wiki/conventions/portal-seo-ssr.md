...
- Entries: [`apps/portal/src/entry-client.tsx`](../../apps/portal/src/entry-client.tsx) (`hydrateRoot`) and [`apps/portal/src/entry-server.tsx`](../../apps/portal/src/entry-server.tsx) (`render` / `renderPage`: `StaticRouter` + `HelmetProvider` + per-request `i18n.cloneInstance` + `SsrResponseContext`).
...
- Guard rails: `src/test/SsrRenderSmoke.test.tsx` (node env) must keep rendering every public route via `renderToString`.
