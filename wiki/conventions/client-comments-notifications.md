...
- Add/**reply**/**edit**/like/delete/report persist via `src/lib/comments` in mock (Impl 62, 197). HTTP mode uses [`portal-comments-http.md`](portal-comments-http.md) (`GET /api/comments`, cookie POSTs). No optimistic local clones inside `Comments` — the component is **fully controlled**: props in, callbacks out.
- Char limit **500**. Count = all rows including replies. `spoiler?` / `reported?` are additive on `StoredComment` (schema stays `v1`). Report flags the row; it stays visible. No admin dashboard.
- Mock `addReply` creates `comment_reply` for the parent author (not self; respects `commentReply` pref) then `reloadInbox`. HTTP reply notifications are created **on the API** only. `titleKey` is `notificationsPage.commentReply` (inbox `t(titleKey)`). Frozen body: mock `i18n.t('notificationsPage.commentReplyBody')` at create; API stores the English of that key.
- Reader overlay is `CommentsSheet` (mobile bottom sheet / desktop drawer). Header count, last-panel teaser, and sheet share `useCommentsThread`. Hub stays **inline** with `seriesCommentKey`. Empty seed (no fake “Book Lover” list). Demo sticker tray appends unicode into content.
...
`CommentsThread` (Reader + hub) subscribes to `softgate_comments_v1` and `EngagementContext` to `softgate_engage_v1` + `softgate_notifications_v1` + `softgate_notif_prefs_v1` via `useStorageSync` — comments/likes/notifications posted in another tab appear without a refresh. HTTP mode drops the comments, inbox, and prefs keys from that list (API is SoT). The `storage` event never fires in the writing tab, so same-tab optimistic updates stay single-source (`setNotifPrefs` on the context calls `refresh` in mock).
