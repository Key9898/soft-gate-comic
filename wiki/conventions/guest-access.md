...
5. Guest UI chrome: Navigation hides the notification bell and shows Login/Register; Home hides the Continue Reading rail when there is no history. When Continue is empty (guests and members with no history), Home fills that slot with a **Start here** catalog rail (`home.startHere`) — first episode free, Hero ids excluded, cards open `/read/:id/1`. Ranking stays a chart (`home.rankingDesc`); it does not reuse the Start here eyebrow. Continue and Start here never show together. The bottom Get started free CTA stays.
