...
`darkMode` no longer selects literal classes directly (issue #35, [ADR 014](../decisions/014-immersive-theme-scope.md)). `ReaderPage.tsx`'s root sets `data-theme="immersive"` when `darkMode` is true and omits the attribute when false; the twelve semantic colour tokens declared in `apps/portal/src/index.css` re-bind under that attribute, so the reader's surfaces, text, edges, and danger states follow the same preference through CSS instead of markup branching. A handful of chrome sites with no token equivalent (translucent chrome-bar fills, floating-nav opacity/ring, hover-borders) still branch on `darkMode` directly, each documented inline at its call site.
