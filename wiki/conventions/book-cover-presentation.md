...
Ancestor `overflow: hidden` / Framer wrappers (including opacity-only `motion.div`) flatten or mute CSS 3D. Book column wrappers = plain `div`s — **never** wrap `HeroBook3D` in Framer or Tailwind `translate-*` / `transform`. Home Spotlight may fade title/CTAs with motion; the width wrapper’s child is the `.hero-book-enter` shell, which contains `.hero-book-scene` (Impl 99). Home pair offset is **`lg:mt-12` on the row** plus **`lg:mt-4` on the book wrapper** (do not use the same `mt` on both columns). About clips its masthead radial wash in a dedicated `overflow-hidden` layer — **not** on the page root (Impl 85).
