...
Auth-gated Home shelf driven by `softgate_engage_v1` history (`EngagementContext`). Guests never see the rail. Empty authenticated history → omit the section (no placeholders).
...
Home Continue and Library History both use the blended helper. (`episodeProgressPercent` was removed in Impl 64 — test-only dead code.)
