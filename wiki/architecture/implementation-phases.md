...
| 229  | 2026-09-15 | WEBTOON badge erased from `cyber-dreams` by masked push-pull fill (#28)        | [2026-09-15-cyber-dreams-badge-erased.md](../notes/2026-09-15-cyber-dreams-badge-erased.md)                                                                   |
| 230  | 2026-09-15 | Credit placeholders erased from `the-last-horizon` (#28)                       | [2026-09-15-last-horizon-credits-erased.md](../notes/2026-09-15-last-horizon-credits-erased.md)                                                               |
...
Public Hono `GET /api/faq` and `GET /api/cookies` read Admin FAQ/Cookies CMS tables on shared Postgres (schema copy, no website FAQ/Cookies migration). Envelope `{ data }`. Never 401. Stub/`P2021` fail-open to `STUB_FAQ` / `STUB_COOKIES`. Delete-all stays empty (meta present + empty lists). Portal `/faq` `/cookies` fetch when mock is off; fail or mock keeps today’s i18n. Help hub `FAQ_POPULAR_IDS` stays catalog i18n. Cookie CMP / Privacy / Terms unchanged. Admin writes stay Admin Impl 66; this repo does not claim that number. **207** stays unused. Next is **222**. Note: [2026-09-10-faq-cookies-consume.md](../notes/2026-09-10-faq-cookies-consume.md). Convention: [portal-faq-cookies-read.md](../conventions/portal-faq-cookies-read.md), [legal-pages.md](../conventions/legal-pages.md).

---

## Impl Phase 214 — Phase 0 ship blockers from the UI/UX plan (2026-09-14)

**Status:** Done

Eight blockers from Phase 0 of the [UI/UX improvement plan](../notes/2026-09-14-uiux-improvement-plan.md), tracked as GitHub issues #3–#10 under epic #17. Series detail hero gets `w-full min-w-0` so the six-card stats row stops forcing the column past the viewport at 375px. Reader unlock carries the price on the CTA, a balance-after line, an inline confirm step, a pending state, and a root `role="status"` announcement. Insufficient coins no longer navigates away: the paywall renders the shortfall and a top-up link carrying `{ from, needCoins, episodeNumber, seriesTitle }`, which `/coins` reads into a context strip with the smallest sufficient pack and a return link. `LibraryDeleteConfirmDialog` is promoted to `components/ConfirmDialog` with optional type-to-confirm; account deletion and both notification deletes now route through it. Demo card fields are `readOnly` and pre-filled from `DEMO_CARD`; `handleCopyMerchant` is guarded. New `--color-muted` / `--color-muted-strong` tokens (gray-600 / gray-700) replace `text-gray-400` on every light surface; dark surfaces keep gray-400. Reduced motion now covers the hero-book scene, while the radial dialog enter animation stays per [forced-product-motion.md](../conventions/forced-product-motion.md). 29 episode strips stop borrowing other series' covers, and `isPlaceholderStrip` drives a labelled demo note in the reader. Binary assets (`banner.png` wordmarks, `[AUTHOR NAME]` on cover art) are **not** fixed — they need real artwork and issue #4 stays open. Notification undo is not implemented: the store has no restore path. **207** stays unused. Next is **215**. Note: [2026-09-14-phase-0-ship-blockers.md](../notes/2026-09-14-phase-0-ship-blockers.md). Plan: [2026-09-14-uiux-improvement-plan.md](../notes/2026-09-14-uiux-improvement-plan.md).

---

## Impl Phase 215 — Phase 1 design-system core (2026-09-14)

**Status:** Done

First tranche of Phase 1 (GitHub epic #18, issue #11). `components/Button/buttonClasses.ts` becomes the single class builder and every size clears the 44pt floor (`sm`/`md` `min-h-11`, `lg` `min-h-12`); `ButtonLink` and `ButtonAnchor` let a navigating CTA be a Button, which was the actual reason CTA class strings kept being hand-rolled. New `surface` variant for the bordered white control; `focus:ring` became `focus-visible:ring`. `features/info/components/infoStyles.ts` collapses the CTA/card/heading/rule constants — `PRIMARY_CTA` had existed in three incompatible forms — while **keeping** the info pages' uppercase editorial register as a deliberate difference from the app CTA. `components/Card` finally provides the `Card.tsx` the radius convention already referenced (`card` 2xl, `panel` 3xl). `components/SortMenu` replaces two divergent implementations and adds `menuitemradio`, arrow-key roving with Home/End, opening focus on the current selection, and Escape with focus restore. `components/SearchField` replaces the byte-identical Help/FAQ inputs. `index.css` drops eleven dead component classes and replaces brand/grey hex with theme tokens; the slate book-material literals stay by design. **Correction:** the audit's "63 radius violations" wrongly counted 46 correct `rounded-3xl` panel uses; only 15 are real. Later tranches added `components/Chip`/`ChipLink` (Search chips had been 38px with no state exposed), real `tablist`/`tab`/`tabpanel` semantics on Library, one `Input` with `stacked`/`floating` variants replacing **four** implementations — fixing an id collision where two fields labelled "Password" shared `id="password"` — all 15 radius violations, and the last 10 raw CTA copies. Sub-44px targets on Home went 17/86 to 3/85. Two audit counts were corrected rather than chased: the `min-h` patch count rose to 231 because patches are what make raw controls meet the floor, and the eleven skeleton files are not a defect (the big ones mirror live chrome per Impl 210) — the defect was `ProfilePageSkeleton` drifting from its page, now realigned. Weight hierarchy is deferred to Phase 5, which owns the typeface it depends on. **207** stays unused. Next is **216**. Note: [2026-09-14-phase-1-design-system-core.md](../notes/2026-09-14-phase-1-design-system-core.md). Convention: [border-radius.md](../conventions/border-radius.md).
...
## Impl Phase 221 — Rail labels renamed (2026-09-15)

**Status:** Done

Closes the item left open on GitHub #21. Phase 4 declined to collapse the six discovery rails — each has its own selector in `discovery.ts` and one is personalised — and proposed renaming instead so the negative disclaimers become unnecessary. Owner chose: Popular → **Most read**, Trending Now → **Rising this week**, Updated → **New episodes**, New Releases → **New series**. All four decks lose their "not …" clauses: "New episodes" and "New series" now distinguish themselves. Each label was checked against its selector first, because a label that overclaims is worse than a disclaimer — `rankingWebtoons` is cumulative `viewCount`, `trendingWebtoons` is `weeklyViewCount`, `newReleaseWebtoons` is `createdAt`, and `updatedWebtoons` is `updatedAt` with New Releases ids excluded (the exclusion is exactly why the old deck said "not new series"). `home.ranking` and `home.newReleases` are not rail-only, so the nav links, Categories sort options, destination tiles on 404/Author/Categories and the `/ranking` heading all follow — deliberately, since the nav saying "Popular" while its rail says "Most read" would be worse. `categories.recentlyUpdated` also became "New episodes", because the rail's View All landed on a sort control naming the same thing differently. Start here, For You and Daily untouched. 57 assertions across 10 test files and 8 test names updated; `SortMenu.test.tsx` skipped as its "Popular" is a local fixture. **207** stays unused. Next is **222**. Note: [2026-09-15-rail-label-rename.md](../notes/2026-09-15-rail-label-rename.md).
...
## Impl Phase 229 — WEBTOON badge erased from the Cyber Dreams cover (2026-09-15)

**Status:** Done (last of #28's three branded covers; artwork still open)

Entry written retroactively — `3a89667` shipped as code with no wiki row, and the figures below were re-measured against `3a89667^` rather than copied from the commit. [Impl 224](#impl-phase-224--competitor-branding-removed-from-two-covers-2026-09-15) deliberately left this cover alone: the badge is large, opaque and white over detailed wet asphalt between both boots, and blurring it produced a pale smear worse than the logo. **Blur was the wrong operation, not a badly tuned one** — it averages the white badge with its surroundings, so white survives as haze at any radius, which is why no sigma could have rescued 224's technique here. Worth separating from 224's earlier sigma dead end, which looked identical and was not: there a 0.45 falloff left 90% of the mask semi-transparent and the fix was the mask; here the mask is fine and the operation is wrong. Removal needs the badge pixels **discarded** and the hole filled from around it, in three steps on the badge and the `SEASON 1` line. **(1) Mask by min-channel threshold at 72%**, dilated, closed, then filtered by connected component to drop anything under 150px — min-channel is what separates the neutral white marks from the boots' cyan trim, which is bright but low in red and which a luminance threshold takes with it; the area filter drops the rain droplets. **(2) Fill by alpha-weighted push-pull rather than blur**: premultiply by the inverse mask, blur both premultiplied image and mask at sigma 6, 14, 30 and 60, divide, then layer coarse to fine weighted by the blurred mask — colour flows in from the surrounding ground and **no white is in the input, so none can survive in the output**, which is what makes the result checkable rather than a matter of taste. **(3) Regrain**, since the fill was plausible in colour but too smooth against the halftone texture elsewhere: a high-pass at sigma 1.2 from a plain patch of the same ground, mirror-tiled. **Step 3 nearly repeated 224's near-miss** — the first donor strip carried a puddle edge that the high-pass kept, pasting a recognisable ring into the patch, invisible at delivery size and obvious at 1:1; narrowing the donor to plain ground fixed it. That is twice a patch resembling its surroundings passed casual review, and Impl 233 then found the rule broken a third time on an encoder change. **The encoding finding is the part 224 needed and did not have**: 224 hit the same question — the source re-encodes larger, what now — and answered it with `palette: true` at a cost of 37.1 dB, where here three candidates were measured. Re-encoding at the source's own 4:2:0 gives 44.6 dB and a worst pixel of 20 of 255 outside the edit, from a second generation of chroma subsampling on the neon edges; **4:4:4 gives 55.8 dB and a worst pixel of 4** at 1.41 MB against 1.12 MB, and lossless PNG24 would have been 2.0 MB. Re-verified here: every pixel differing by more than 2% sits inside `141x117+442+874`, within the claimed `145x120+440+873`; that region held **4,778 pixels over the 72% min-channel threshold before and 0 after**, its brightest min-channel value falling from 100% to 55%; outside it, **54.6 dB with the same worst pixel of 4** (the commit's 55.8 dB differs only in how the region is excluded); and the badge is absent in the 192px and 384px AVIF the browser serves. **Still a stopgap** — the patch reads as soft reflective haze, rain streaks do not continue through it and it is smoother than the ground at 1:1, and it is not the text-free artwork [cover-artwork-brief](../references/cover-artwork-brief.md) asks for. It removes another company's brand from a page we serve, which is worth doing before the artwork arrives. **207** stays unused. Next is **230**. Note: [2026-09-15-cyber-dreams-badge-erased.md](../notes/2026-09-15-cyber-dreams-badge-erased.md).

---

## Impl Phase 230 — Credit placeholders erased from The Last Horizon (2026-09-15)

**Status:** Done (placeholders; title and tagline still need artwork)

Entry written retroactively — `6e47e55` shipped as code with no wiki row, and the figures below were re-measured against `6e47e55^` rather than copied. `the-last-horizon.png` baked `STORY BY [AUTHOR NAME] | ART BY [ARTIST NAME]` into its bottom right corner, an unfilled template shipping on the home page. **A placeholder is not a logo**, which is the framing [Impl 229](#impl-phase-229--webtoon-badge-erased-from-the-cyber-dreams-cover-2026-09-15) does not supply: a logo only has to go away, a credit line has to _say something_, so the first question is whether it can be filled with the truth rather than erased. It cannot — the catalogue holds one `author` for this series and **no artist field at all**, so `ART BY` has no true value to take, and baked credits would contradict [cover-artwork-brief](../references/cover-artwork-brief.md), which puts author and artist among the fields `BookCard` already renders as live text. **Two things differed from 229 and each cost an attempt.** First, **the text is not neutral**: the placeholders are gold, and 229's min-channel threshold misses gold entirely, since gold is bright in red and green and dark in blue — min-channel was never a general technique, only the right answer to one cover. A luminance threshold at 62% catches both colours but also catches the crystal cluster beside `STORY BY`, so the mask is confined to two boxes. Second, **the text has a drop shadow**, which is why dilating by 3 and then by 7 both left a visibly dark bar; a row-mean luminance profile named the cause rather than guessing — shadow rows measure **0.109 and 0.094** against **0.19** for the rock above and **0.12** below, so the push-pull fill was sampling the shadow as background and filling the hole with it. Dilating to **12** puts the shadow inside the hole instead of the sampling ring, and the filled band then runs 0.198 to 0.124, the rock's own gradient. **No regrain step**, deliberately: 229 needed one for a halftone ground, but this artwork is flat cel-shaded with vector-like outlines and has no grain, so adding any would have invented texture the style does not have — the previous cover's pipeline was not copied wholesale. Re-verified here: all change beyond 2% sits inside `431x84+578+930`, within the claimed `439x90+573+927`; the region held **4,750 pixels over the 62% luminance threshold before and 120 after**, the survivors being crystal tips that were always there; a contrast-stretched view of the fill shows no glyph ghosting; and outside the region **54.9 dB with a worst pixel of 4 of 255** (the commit's 56.2 dB differs only in how the region is excluded — the same ~1.3 dB offset appears on 229 and 231 measured this way). Encoded JPEG q100 4:4:4 at **1.29 MB against 1.05 MB**. **Honest limit**: rock facet outlines that ran under the text are **severed**, so the corner reads as smooth shadow where it read as detailed rock — a loss of detail, not a reconstruction of it. **207** stays unused. Next is **231**. Note: [2026-09-15-last-horizon-credits-erased.md](../notes/2026-09-15-last-horizon-credits-erased.md).

---

## Impl Phase 231 — Title lockup taken off the Love in Seoul cover (2026-09-15)

**Status:** Done (first cover with no baked text at all)

Entry written retroactively — `a2f5e4f` shipped as code with no wiki row, figures re-measured against `a2f5e4f^`. `love-in-seoul.png` baked `서울의 사랑` above `Love in Seoul` on a Myanmar webtoon portal. #28 called the Korean a wrong-language problem, which it is, but **the two lines are one design** — headline, subtitle, and vine and blossom ornaments flanking both on a shared white glow — so removing one line would leave ornaments framing nothing. **It corrected a claim #28 was treating as general**: that a baked title cannot be erased the way a placeholder can, because removing it leaves a hole where the focal element was. Impl 230 repeated that reasoning correctly for its own cover; it is **true where the title is the focal element and false here**, because this lockup sits on open sunset sky with the blossom canopy framing the top, so erasing it leaves sky and the composition reads better for the room. Testing the claim against the specific artwork instead of inheriting it turned a "needs artwork" item into a shipped fix, and this is the **first cover to satisfy** [cover-artwork-brief](../references/cover-artwork-brief.md). Mask is a dark-tone threshold at 55% in a box, dilated by **20** to swallow the white glow — Impl 230's lesson applied, a glow left in the sampling ring fills the hole with glow exactly as a drop shadow did there. **The failure worth recording is in the fill, not the art.** The first pass **looked clean at 1:1**, and an auto-level contrast stretch showed legible ghosts of the Korean glyphs in the filled sky. The cause is arithmetic: deep inside a hole the blurred mask weight falls to roughly **1/255**, and in 8-bit the premultiplied-over-weight divide amplifies that rounding into noise **shaped like the mask**. Running the fill at **16-bit** and trusting a blur scale only where its mask weight exceeds **0.06** removes it. **The 8-bit divide was latent in 229 and 230 too** — both re-probed, neither ghosts — and surfaced here only because this hole is **464x193** against 145x120 and 439x90 and sits on a smooth bright gradient rather than dark texture; sky shows noise that wet asphalt and shadowed rock hide. The generalisable part: **a latent numerical bug surfaces on the largest, smoothest, brightest instance first**, so two covers passing is evidence they were forgiving, not that the arithmetic is sound — and the probe that caught it is one command. Re-verified here: all change beyond 2% sits inside `467x197+272+82`, within the claimed `475x204+268+79`; the region held **13,065 pixels below the 55% dark threshold before and 565 after**, the survivors being the blossom twig; the contrast-stretch probe is clean on this cover and on 229's and 230's patches, confirming neither regressed; outside the region **55.1 dB with a worst pixel of 4 of 255** (commit reports 56.4 dB, same method gap). Encoded JPEG q100 4:4:4 at **1.22 MB against 1.05 MB**. **Honest limit**: the twig tip that ran behind the glow is trimmed and ends about 70px further right, which reads as a twig ending. **207** stays unused. Next is **232**. Note: [2026-09-15-love-in-seoul-lockup-removed.md](../notes/2026-09-15-love-in-seoul-lockup-removed.md).

---

## Impl Phase 232 — What can be erased on the remaining covers, and what cannot (2026-09-15)

**Status:** Done

Worked the rest of [#28](https://github.com/Key9898/soft-gate-comic/issues/28)'s covers after Impl 229–231 did one each. **Three lost text** — `campus-life` (title, tagline, stars, both ornament blossoms and all four character names; now fully text-free), `cyber-dreams` (title and tagline; now fully text-free), `blood-moon` (tagline and `SEASON 1 | WRITTEN & ILLUSTRATED BY NIGHTMARE`, title kept). **Four titles were rejected, each attempted first so the rejection rests on a result** — `blood-moon` sits on the moon's upper limb with its red glow as the lighting (a loose box mask and a tight letterform mask both smeared the moon into a blob), `ocean-dreams` has the ship's mast, rigging and flag behind it (the fill severed the mast and left the flag floating), `forest-spirit` sits on dense canopy (a flat pale-green oval conspicuous even at 183px), and `the-last-horizon` filled flat and was then "fixed" by regrain **pasting a duplicate constellation** into open sky — Impl 224's duplicate temple roof, fourth occurrence. Attempting before rejecting is the point: Impl 224 rejected `cyber-dreams` by inspection and was right, but Impl 229 showed its stated reason was a property of the _operation_, not the artwork, and an unattempted rejection cannot tell those apart. **The rule established: remove text only where the background can be reconstructed from its own surroundings, never where content has to be invented or duplicated.** `cyber-dreams` passes and is the instructive contrast — its regrain donor is a strip of sky _directly above the title in the same columns_, so the rain it restores is that sky's own, at that sky's angle and spacing; same technique as `the-last-horizon`'s, opposite verdict, and **the discriminator is provenance rather than appearance**. A flat blob and a cloned constellation are both failures; the clone is the more dangerous because it does not announce itself, which is why it keeps shipping. **This answers the question Impl 231 opened** — erasability is per-cover, and the answer is four of seven titles are not erasable, in every case because a load-bearing element of the composition runs behind the text: the property that decides it is not the title but what is behind it. Re-measured against `af3abf7^`: change contained to `470x84+277+926`, `639x468+208+50` and `511x214+262+33`; `blood-moon`'s band held **8,782 bright pixels before and 0 after**, `campus-life`'s title box 15,609 dark before and 13 after with all four name boxes going 587/530/342/545 → **0**, and `cyber-dreams`' title box 15,454 → 457 where the survivors are the character's hair entering the box, not a letterform; contrast-stretch probes (Impl 231's, now standard) show no glyph ghosts; outside the regions **56.8 / 57.6 / 59.7 dB**, worst pixel 4 of 255, with `cyber-dreams` taking a **third** JPEG generation at q100 4:4:4 and still getting _smaller_ (1.396 MB against 1.405 MB) — generational loss is not the constraint Impl 224 took it for. **Honest limit worth generalising**: `campus-life`'s `CHLOE` patch landed on the library's arched doorway and lost the arch, the only patch in this set to fall on architecture — on asphalt, rock, canopy or sky a smooth fill reads as atmosphere, on a hard-edged built structure it reads as damage, so provenance decides whether a fill is _possible_ and the background's character decides whether it is _invisible_. `University Library` and `cyber-dreams`' neon shop signs are deliberately kept as signage inside the illustrated world rather than catalogue fields duplicated as artwork. `golden-age` and `shadow-knight` untouched here, being re-rendered from their pre-Impl-224 sources in parallel. **207** stays unused. Next is **233**. Note: [2026-09-15-remaining-covers-erased.md](../notes/2026-09-15-remaining-covers-erased.md).

---

---

## Impl Phase 233 — Two covers restored to full colour (2026-09-15)

**Status:** Done (encoding half of #28's branding item; artwork still open)

[Impl 224](#impl-phase-224--competitor-branding-removed-from-two-covers-2026-09-15) erased the "Webtoon Original" wordmarks correctly and then wrote both covers back as **8-bit colormap PNG**. **The cascade started with never asking what the sources were**: all nine covers are **JPEG q100 4:2:0 carrying a `.png` extension**, so sharp's lossless PNG re-encode was naturally far larger (1037 -> 2653 kB) — a fact about the file already being lossy, not about PNG. Read as a PNG problem, it was answered with `palette: true`, which quantised two neon/gradient illustrations, the worst case, to 256 colours. Measured outside the erased rectangle — i.e. damage to artwork the edit never meant to touch — `golden-age` lost **37.1 dB PSNR with a worst pixel 70 of 255** and `shadow-knight` **39.5 dB / 139 of 255**, across **95%+ of every pixel in both files**; a smooth sky patch went from **7,030 distinct colours to 137**. Impl 224's "no banding" was not wrong about banding, since sharp's quantiser dithers and the artefact is added grain rather than hard steps — it was wrong about what to look for, and looked only at the region it had edited. **The transferable finding is that this is Impl 224's own lesson one level up**: it learned "diff a patch against the original at full resolution" from a clone-stamp that pasted a duplicate temple roof, then broke the rule on the encoder, because re-encoding does not feel like an edit — no region chosen, no pixels drawn, invisible at delivery size, and 95% of the file changed. **An encoder change is an edit to every pixel and earns the same A/B.** This re-applies the erasure to the pre-224 sources: same technique (Gaussian blur under a raw-pixel cosine-falloff alpha), **with the feather in pixels rather than as a fraction of the region** — 224's `0.10 / 0.16` fractions put the outermost glyphs inside the ramp, and reproducing them left a ghost `W` and `l` on `golden-age` that passed at card size and was obvious at 1:1, so the region is now the measured glyph box grown by exactly the ramp width and the glyphs sit in the full-alpha core by construction. Sigma 24; lower sigmas clear the letterforms but leave residue from their drop shadows. Encoded **JPEG q100 4:4:4** per Impl 229, giving **54.5 / 55.0 dB and a worst pixel of 4** at 1.36 / 1.18 MB (against 610/649 kB palette, 1.06/0.95 MB sources) — worth paying because the sources are the `<img>` fallback in `responsiveImage.ts` and every AVIF/WebP rung is derived from them. Verified by full-resolution A/B, by the branding being absent in the delivered `-384.avif`, and by 109 files / 791 tests green. **Impl 229 (`3a89667`, the Cyber Dreams badge removal) shipped as code with no wiki entry**; its row and phase section were written retroactively alongside this one. **Impl 230 (`6e47e55`) and 231 (`a2f5e4f`) then landed on `development` the same way — code only** — while this work was in review; both were written retroactively alongside it. **Impl 232 (`af3abf7`) then did the same again**, landing between this branch's last push and its merge, which is why this phase is 233 and not 232 — the second renumber this one change has needed. Four consecutive phases have now shipped undocumented; 229, 230 and 231 are on the record, 232 is rowed as unwritten. Their covers were checked for the defect this phase fixes and are clean: `the-last-horizon` and `love-in-seoul` are both **JPEG q100 4:4:4** at 291,801 and 151,908 colours, i.e. they followed Impl 229's encoding rather than 224's. **207** stays unused. Next is **234**. Note: [2026-09-15-cover-encoding-restored.md](../notes/2026-09-15-cover-encoding-restored.md).
