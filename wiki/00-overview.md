...
- **API:** Portal mock/localStorage when `VITE_USE_MOCK_API` is not `false` (Vercel unset). Committed example is `false`; local `pnpm dev` HTTP needs gitignored `.env.development.local` + `pnpm dev:api`. `apps/api` Hono (`GET /health`, `GET /api/catalog`, `GET /api/settings`, reader `/api/auth/*`, `/api/wallet/*`, `/api/library/*`, `/api/notifications/*`, `/api/prefs/*`; persist stub or Prisma when `DATABASE_URL` is set — Impl 185; R2 put helper under `portal/` — Impl 186; Brevo forgot/reset — Impl 187; profile writers — Impl 188) — Impl 171–195. Leader-dev JWT/R2/Brevo map onto existing slots in gitignored env (Impl 193). Local `DATABASE_URL` + migrate once + health `"prisma"` (Impl 194). Prisma catalog from Admin tables when persist is Prisma (Impl 195). Git: `main` is product/Vercel; `development` is the leader-dev integration track.
