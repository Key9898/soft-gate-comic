...
- **Outside the edited regions: 56.8, 57.6 and 59.7 dB**, worst pixel 4 of 255 — measured away from the patch edge. Immediately outside the patches these three read **8, 8 and 6**, the only phase in 229–234 that does; it is a 1–3 pixel rim of JPEG blocks straddling the boundary, a few dozen pixels in all, and it is back to 4 sixteen pixels out. See [cover-patch-measurement](../conventions/cover-patch-measurement.md). `cyber-dreams` takes a **third** JPEG generation at q100 4:4:4 and still measures 59.7 dB — and got _smaller_, 1.396 MB against 1.405 MB. Generational loss at these settings is not the constraint it looked like in Impl 224.
