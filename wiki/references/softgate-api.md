...
Passwords hashed with native `bcrypt` (cost 12 in production, 4 under `NODE_ENV=test`; Impl 222). httpOnly cookies `sg_reader` (15 min) and `sg_reader_refresh` (7 days). Errors `{ error: { code } }`. Register 403 `REGISTRATION_CLOSED` when settings close registration or maintenance is on. Users live in the persist adapter (stub maps or Prisma `ReaderUser`). Forgot always `{ data: { ok: true } }`. Reset 400 `RESET_TOKEN_INVALID`; no session cookies. Profile is a partial patch (`displayName`, `email`, `bio`, `avatar`); empty `{}` → 400; email taken → 409. `avatar` present must be a jpeg/png/webp data URL within the 512 KB string-length cap or 400 `VALIDATION_ERROR`. Password change does not re-issue cookies. Delete-account clears cookies.
