...
- [02-workflow.md](02-workflow.md) — git workflow (`main` / `development`), hooks, documentation dual-track
- [03-folder-map.md](03-folder-map.md) — codebase folder responsibilities

### Architecture & references

- [architecture/implementation-phases.md](architecture/implementation-phases.md) — SoftGate Comic Impl master (**next: 234**)
...
- [references/softgate-api.md](references/softgate-api.md) — SoftGate `apps/api` health + catalog + settings + about + press + reader auth + wallet + library + notifications + prefs + comments + Prisma persist + R2 helper + Brevo forgot/reset + profile writers + leader-dev env mapping + Admin catalog read (Impl 171–198) + Admin PlatformSettings read (Impl 200) + Admin CoinPackage on catalog (Impl 201) + live join smoke (Impl 202) + notification delivery pipe (Impl 203) + new_episode fan-out (Impl 204) + public about read (Impl 205) + portal History consume (Impl 206) + portal Team consume (Impl 207) + public press read (Impl 211) + public Privacy/Terms read (Impl 212) + public FAQ/Cookies read (Impl 213)
...
- [conventions/type-weight-scale.md](conventions/type-weight-scale.md) — Inter-only weight hierarchy + cover-overlay exception (Impl 220)
...
- [conventions/hero-spotlight.md](conventions/hero-spotlight.md) — Home Spotlight rotator (Impl 49, jobs Impl 104); Home loading banner + pageHeading only (Impl 210); empty slides keep banner + site heading (Impl 196); load-fail omits Help/Creators (Impl 198)
- [conventions/forced-product-motion.md](conventions/forced-product-motion.md) — product animations we add are forced (Impl 90); auth photo curtain (Impl 149); comments sheet enter (Impl 197); reader chrome + ReaderSheet enter (Impl 199); Home rail radial View All (Impl 208)
...
- [conventions/portal-press-read.md](conventions/portal-press-read.md) — public `GET /api/press` + PressPage consume (Impl 211); i18n fallback on mock/fail
- [conventions/portal-legal-read.md](conventions/portal-legal-read.md) — public `GET /api/legal/privacy` + `GET /api/legal/terms` + Privacy/Terms consume (Impl 212); i18n fallback on mock/fail
- [conventions/portal-faq-cookies-read.md](conventions/portal-faq-cookies-read.md) — public `GET /api/faq` + `GET /api/cookies` + FAQ/Cookies consume (Impl 213); i18n fallback on mock/fail
...
- [conventions/portal-notifications-http.md](conventions/portal-notifications-http.md) — mock inbox key vs `/api/notifications` (Impl 191); push siblings (Impl 203); HTTP stops client `new_episode` scan (Impl 204)
- [conventions/portal-notifications-deliver.md](conventions/portal-notifications-deliver.md) — Admin service-token broadcast + Brevo + Web Push + comment_reply out-of-band (Impl 203); `new-episode` fan-out (Impl 204)
- [conventions/portal-prefs-http.md](conventions/portal-prefs-http.md) — mock notif/reader prefs keys vs `/api/prefs` (Impl 192); HTTP no client `new_episode` scan (Impl 204)
...
- [conventions/named-integrations.md](conventions/named-integrations.md) — Prisma persist when `DATABASE_URL` is set; R2 put helper under `portal/` (Impl 186); Brevo forgot/reset when key+from set (Impl 187); leader-dev slot mapping (Impl 193); local Prisma URL + migrate once (Impl 194); Prisma catalog from Admin tables (Impl 195); Prisma PlatformSettings (Impl 200); Prisma CoinPackage (Impl 201); live join port (Impl 202); Admin service token + VAPID (Impl 203); new-episode ping reuses token (Impl 204); Prisma About CMS read (Impl 205); portal History consume (Impl 206); portal Team consume (Impl 207); Prisma Press CMS read (Impl 211); Prisma Privacy/Terms CMS read (Impl 212); Prisma FAQ/Cookies CMS read (Impl 213); health is listen proof
...
- [conventions/library-bookmarks.md](conventions/library-bookmarks.md) — Subscribe → Library store (Impl 25, rename Impl 140); History Continue (Impl 142); HTTP new_episode from Admin ping (Impl 204)
...
- [conventions/discovery-honesty.md](conventions/discovery-honesty.md) — author→profile, related, LS version (Impl 13–15, 135); Search Demo chips not live trends (Impl 145); For You (Impl 147); Author Follow not public count (Impl 148); Daily `uploadDay` (Impl 150); Daily scheduled drops (Impl 153); Demo Updated/New dates + copy (Impl 154); trust stored catalog (Impl 167); empty catalog is empty chrome not seed (Impl 196); load-fail is not unpublished (Impl 198); Home Popular/Updated/New View All radial dialog (Impl 208); catalog loading wells not book grids (Impl 210)
...
- [conventions/client-comments-notifications.md](conventions/client-comments-notifications.md) — comments + notifs (Impl 34–35); empty-first inbox + prefs (Impl 142); series hub thread (Impl 144); shared HTTP + comment_reply (Impl 197); one switch + out-of-band (Impl 203); HTTP new_episode from Admin ping (Impl 204)
- [conventions/typography.md](conventions/typography.md) — Inter + Noto Sans Myanmar stack (Impl 12)
- [conventions/info-page-chrome.md](conventions/info-page-chrome.md) — breadcrumb + PageHeader tiers (Impl 17); Creators intake (Impl 107, polish 110); Support funnel (Impl 108, 111); Contact pitch fields (Impl 112); 404 recovery (Impl 118, polish 121); Help hub + Footer FAQ (Impl 120); Press kit ZIP + Demo IA (Impl 209); Press live GET /api/press (Impl 211); Privacy/Terms live GET (Impl 212); FAQ/Cookies live GET (Impl 213)
- [conventions/source-layering-and-imports.md](conventions/source-layering-and-imports.md) — layers + hybrid imports (Impl 19)
- [conventions/legal-pages.md](conventions/legal-pages.md) — legal document shell + storage honesty (Impl 59–60, layered 126); TOC scrollbar (Impl 128); notif prefs row (Impl 142); episode reports row (Impl 199); Privacy/Terms live GET (Impl 212); Cookies live GET (Impl 213)
...
- [notes/2026-09-09-platform-settings-read.md](notes/2026-09-09-platform-settings-read.md) — Impl 200 read Admin PlatformSettings on GET /api/settings
- [notes/2026-09-10-coin-packages-catalog.md](notes/2026-09-10-coin-packages-catalog.md) — Impl 201 read Admin CoinPackage on catalog + /coins
- [notes/2026-09-10-live-join-smoke.md](notes/2026-09-10-live-join-smoke.md) — Impl 202 live join smoke (catalog + settings + coins + reader)
- [notes/2026-09-10-live-join-api-listen.md](notes/2026-09-10-live-join-api-listen.md) — live join: catalog fail + CONNECTION_REFUSED = SoftGate not listening
- [notes/2026-09-10-notification-delivery-pipe.md](notes/2026-09-10-notification-delivery-pipe.md) — Impl 203 reader notification delivery pipe
- [notes/2026-09-10-new-episode-fan-out.md](notes/2026-09-10-new-episode-fan-out.md) — Impl 204 new_episode API fan-out
- [notes/2026-09-10-about-public-read.md](notes/2026-09-10-about-public-read.md) — Impl 205 public GET /api/about
- [notes/2026-09-10-about-history-ui.md](notes/2026-09-10-about-history-ui.md) — Impl 206 Portal Our history from GET /api/about
- [notes/2026-09-10-about-team-ui.md](notes/2026-09-10-about-team-ui.md) — Impl 207 Portal Our team from GET /api/about
- [notes/2026-09-10-home-rail-radial-view-all.md](notes/2026-09-10-home-rail-radial-view-all.md) — Impl 208 Home rail Radial Cards View All
- [notes/2026-09-10-press-kit-complete.md](notes/2026-09-10-press-kit-complete.md) — Impl 209 complete Press kit (real ZIP + Demo IA)
- [notes/2026-09-10-catalog-loading-chrome.md](notes/2026-09-10-catalog-loading-chrome.md) — Impl 210 honest catalog loading chrome
...
- [notes/2026-09-14-phase-2-funnel-retention.md](notes/2026-09-14-phase-2-funnel-retention.md) — Impl 217 Phase 2 funnel (empty-state causes, wallet pre-flight, end-of-series, guest reason)
- [notes/2026-09-14-phase-4-discovery-ia.md](notes/2026-09-14-phase-4-discovery-ia.md) — Impl 218 Phase 4 IA (View all destinations, one failure message, offline; 4 review findings corrected)
