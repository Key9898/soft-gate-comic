...
- [architecture/implementation-phases.md](architecture/implementation-phases.md) — SoftGate Comic Impl master (**next: 205**)
...
- [references/softgate-api.md](references/softgate-api.md) — SoftGate `apps/api` health + catalog + settings + reader auth + wallet + library + notifications + prefs + comments + Prisma persist + R2 helper + Brevo forgot/reset + profile writers + leader-dev env mapping + Admin catalog read (Impl 171–198) + Admin PlatformSettings read (Impl 200) + Admin CoinPackage on catalog (Impl 201) + live join smoke (Impl 202) + notification delivery pipe (Impl 203) + new_episode fan-out (Impl 204)
