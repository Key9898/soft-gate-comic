...
const ReaderSkeleton = ({
  ...
  // Border tokenises (edge); the translucent chrome bg + shadow have no token
  // equivalent and stay branched.
  const chromeBorder = `border-edge ${
    darkMode
      ? 'bg-gray-950/75 shadow-[0_8px_32px_0_rgba(0,0,0,0.37)]'
      : 'bg-white/75 shadow-[0_8px_32px_0_rgba(31,38,135,0.08)]'
  }`
  const footerChromeBorder = `border-edge ${
    darkMode
  ...
  return (
    // data-theme scopes the skeleton's own tokens to the reader's dark preference; the
    // Skeleton/SkeletonText placeholder colour comes from the separate `tone` variant
    // prop (Skeleton.tsx), not from these tokens, and stays as-is.
    <div data-theme={darkMode ? 'immersive' : undefined}>
      <SkeletonSection className="bg-canvas min-h-screen">
        <header
