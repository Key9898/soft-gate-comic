...
export function createApp(env: Env, options?: { mail?: MailPort; push?: PushPort }) {
  ...
  app.get('/api/about', async (c) => c.json({ data: await persist.getAbout() }))
  app.get('/api/press', async (c) => c.json({ data: await persist.getPress() }))
  app.get('/api/legal/privacy', async (c) => c.json({ data: await persist.getPrivacy() }))
  app.get('/api/legal/terms', async (c) => c.json({ data: await persist.getTerms() }))
  app.get('/api/faq', async (c) => c.json({ data: await persist.getFaq() }))
  app.get('/api/cookies', async (c) => c.json({ data: await persist.getCookies() }))
